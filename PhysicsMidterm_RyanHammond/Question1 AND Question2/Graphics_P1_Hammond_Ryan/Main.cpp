...
glm::vec3 g_offset = glm::vec3(-20.0f, 5.0f, -20.f);
...
int main(void)
{
	...
	while (!glfwWindowShouldClose(gWindow))
	{
		...
		cSphere* p_yWing = findObjectByID(g_yWingID);
		if (wPressed != sPressed)
		{
			if (sPressed) cameraSpeed *= -1;
			p_yWing->position.z += cameraSpeed;
			p_yWing->position.x += cameraSpeed;
			g_targetTransform[3] = glm::vec4(p_yWing->position, 1.0f);//glm::translate(g_targetTransform, glm::vec3(cameraSpeed, 0.0, 0.f));
		}
		if (aPressed != dPressed && !shift)
		{
			if (dPressed) turnSpeed *= -1;
			p_yWing->position.x += turnSpeed;
			g_targetTransform[3] = glm::vec4(p_yWing->position, 1.0f);
			
		}
		if (aPressed != dPressed && shift)
		{
			if (dPressed) turnSpeed *= -1;
			g_targetTransform = glm::rotate(g_targetTransform, 0.1f, glm::vec3(0.f, turnSpeed, 0.f));
		}
...
void CollisionStep(float deltaTime)
{
	...
	for (int indexOut = 0; indexOut != ::g_vec_pSpheres.size(); indexOut++)
	{
	 ...
	 // ADDED: IF there is no groundCollisionFaderCounter...
	for (int index = 0; index != ::g_vec_pSpheres.size(); index++)
	{
		...
		if (pCurGO->groundCollisionFadeCounter <= 0.0f)
		{
			...
			pCurGO->solidColour.g = 0.1f;
			pCurGO->solidColour.b = 0.1f;
		}
		else
	 ...
	 // 2. For each ball, check for collision with triangles (environment)
	 // 
	for (int ballIndex = 0; ballIndex != ::g_vec_pSpheres.size(); ballIndex++)
	{
		...
		for (int triMeshIndex = 0; triMeshIndex != g_vec_pTriangleMeshes.size(); triMeshIndex++)
		{
			...
			meshTriInfo = g_vec_pTriangleMeshes[triMeshIndex]->physicsTris;	//change this to the object triangle info
		...
		// Go through all the triangles, checking for collisions
			for (int triIndex = 0; triIndex != meshTriInfo.size(); triIndex++)
			{
				...
				{	// Bazinga!! 
					...
					curContact.normal = normal;
					curContact.objectID = pBall->getID();
					curContact.collisionType = cContact::WITH_TRIANGLE;

					//pBall->position = pBall->lastPosition;
					//pBall->velocity = glm::reflect(pBall->velocity, normal);
	...
	if (!::g_vec_Contacts.empty())
	{
		...
		int test = g_vec_pSpheres.back()->getID();
		for (int IDIndex = 0; IDIndex < g_vec_pSpheres.back()->getID(); IDIndex++)
		{
			std::vector<cContact> curContact;
			for (int index = 0; index != ::g_vec_Contacts.size(); index++)
			{
				...
				if (ID == g_vec_pSpheres[IDIndex]->getID())
				{
					curContact.push_back(g_vec_Contacts[index]);
				}
			...
			{
				cSphere* pCurGO = 0;
				glm::vec3 responseVelocity;
				int contactIndex = 0;
				for (contactIndex = 0; contactIndex != curContact.size(); contactIndex++)
				{
					pCurGO = findObjectByID(curContact[contactIndex].objectID);

					if (curContact[contactIndex].collisionType == cContact::eCollisionType::WITH_SPHERE)
					{
						pCurGO->solidColour.r = 1.0f;
						pCurGO->solidColour.g = 0.0f;
						pCurGO->solidColour.b = 0.0f;
						responseVelocity += curContact[contactIndex].otherVelocity;
					}
					else if (curContact[contactIndex].collisionType == cContact::eCollisionType::WITH_TRIANGLE)
					{

						responseVelocity += glm::reflect(pCurGO->velocity, curContact[contactIndex].normal);
					}
					if (pCurGO->groundCollisionFadeCounter <= 0.0f)
					{
				...
				pCurGO->position = pCurGO->lastPosition;
				pCurGO->velocity = glm::vec3(responseVelocity.x / contactIndex, responseVelocity.y / contactIndex, responseVelocity.z / contactIndex);
			}
		}
	}//if ( ! ::g_vec_Contacts.empty() )
...
{
	glm::vec3 gravityForce(0.0f, -9.8, 0.0f);
...
void highlightTurrets()
{
	...
	g_vec_pTriangleMeshes.erase(g_vec_pTriangleMeshes.begin() + 101, g_vec_pTriangleMeshes.end());
